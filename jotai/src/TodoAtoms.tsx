...
const TodoItem = ({ todoAtom, remove }: { todoAtom: TodoAtom; remove: (todoAtom: TodoAtom) => void }) => {
...
const TodoList = () => {
  ...
  const remove = useCallback(
    (todoAtom: TodoAtom) => setTodoAtoms((prev) => prev.filter((item) => item !== todoAtom)),
    [setTodoAtoms],
  );
  return (
    <div>
...
const TodoAtoms = () => (
  <>
    <h3>Jotai의 Atoms-in-Atom을 이용한 Todo리스트</h3>
    <TodoList />
    <NewTodo />
  </>
);
